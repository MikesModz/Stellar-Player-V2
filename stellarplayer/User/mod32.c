...
static const uint16_t amigaPeriods[296] = { 907, 900, 894, 887, 881, 875, 868,
		862, //  -8 to -1
...
static void loadHeader() {
	...
	for (i = 0; i < SAMPLES; i++) {
		...
		if (uMod.Mod.samples[i].fineTune > 7)
			uMod.Mod.samples[i].fineTune -= 16;
		...
		if (uMod.Mod.samples[i].loopBegin + uMod.Mod.samples[i].loopLength
				> uMod.Mod.samples[i].length)
			uMod.Mod.samples[i].loopLength = uMod.Mod.samples[i].length
					- uMod.Mod.samples[i].loopBegin;
	}
...
static void loadSamples() {
	uint8_t i;
	uint32_t fileOffset = 1084
			+ uMod.Mod.numberOfPatterns * ROWS * uMod.Mod.numberOfChannels * 4
			- 1;
	...
	for (i = 0; i < SAMPLES; i++) {
		...
		if (uMod.Mod.samples[i].length) {
			...
			if (uMod.Mod.samples[i].loopLength > 2) {
				...
				Mixer.sampleLoopEnd[i] = Mixer.sampleLoopBegin[i]
						+ Mixer.sampleLoopLength[i];
			} else {
...
static void loadPattern(uint8_t pattern) {
	...
	for (row = 0; row < ROWS; row++) {
		for (channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
			...
			uPlayer.Mod_player.currentPattern.sampleNumber[row][channel] =
					(temp[0] & 0xF0) + (temp[2] >> 4);
			...
			for (i = 1; i < 37; i++)
				if (amigaPeriod > amigaPeriods[i * 8] - 3
					...
					uPlayer.Mod_player.currentPattern.note[row][channel] = i
							* 8;

			uPlayer.Mod_player.currentPattern.effectNumber[row][channel] =
					temp[2] & 0xF;
			uPlayer.Mod_player.currentPattern.effectParameter[row][channel] =
					temp[3];
...
static void portamento(uint8_t channel) {
	if (uPlayer.Mod_player.lastAmigaPeriod[channel]
			< uPlayer.Mod_player.portamentoNote[channel]) {
		uPlayer.Mod_player.lastAmigaPeriod[channel] +=
				uPlayer.Mod_player.portamentoSpeed[channel];
		if (uPlayer.Mod_player.lastAmigaPeriod[channel]
				> uPlayer.Mod_player.portamentoNote[channel])
			uPlayer.Mod_player.lastAmigaPeriod[channel] =
					uPlayer.Mod_player.portamentoNote[channel];
	...
	Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
			/ uPlayer.Mod_player.lastAmigaPeriod[channel];
}
...
static void vibrato(uint8_t channel) {
	...
	if (uPlayer.Mod_player.vibratoPos[channel] >= 0)
		Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
				/ (uPlayer.Mod_player.lastAmigaPeriod[channel] + delta);
	else
		Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
				/ (uPlayer.Mod_player.lastAmigaPeriod[channel] - delta);

	uPlayer.Mod_player.vibratoPos[channel] +=
			uPlayer.Mod_player.vibratoSpeed[channel];
	if (uPlayer.Mod_player.vibratoPos[channel] > 31)
		uPlayer.Mod_player.vibratoPos[channel] -= 64;
}
...
static void tremolo(uint8_t channel) {
	...
	if (uPlayer.Mod_player.tremoloPos[channel] >= 0) {
		if (uPlayer.Mod_player.volume[channel] + delta > 64)
			delta = 64 - uPlayer.Mod_player.volume[channel];
		Mixer.channelVolume[channel] = uPlayer.Mod_player.volume[channel]
				+ delta;
	} else {
		if (uPlayer.Mod_player.volume[channel] - delta < 0)
			delta = uPlayer.Mod_player.volume[channel];
		Mixer.channelVolume[channel] = uPlayer.Mod_player.volume[channel]
				- delta;
	}

	uPlayer.Mod_player.tremoloPos[channel] +=
			uPlayer.Mod_player.tremoloSpeed[channel];
	if (uPlayer.Mod_player.tremoloPos[channel] > 31)
		uPlayer.Mod_player.tremoloPos[channel] -= 64;
}
...
static void processRow() {
	...
	for (channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
		...
		sampleNumber =
				uPlayer.Mod_player.currentPattern.sampleNumber[uPlayer.Mod_player.lastRow][channel];
		note =
				uPlayer.Mod_player.currentPattern.note[uPlayer.Mod_player.lastRow][channel];
		effectNumber =
				uPlayer.Mod_player.currentPattern.effectNumber[uPlayer.Mod_player.lastRow][channel];
		effectParameter =
				uPlayer.Mod_player.currentPattern.effectParameter[uPlayer.Mod_player.lastRow][channel];
		...
		if (sampleNumber) {
			...
			if (!(effectParameter == 0xE && effectParameterX == MOD32_NOTEDELAY))
				uPlayer.Mod_player.volume[channel] =
						uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].volume;
		}

		if (note != NONOTE_MOD) {
			uPlayer.Mod_player.lastNote[channel] = note;
			uPlayer.Mod_player.amigaPeriod[channel] =
					amigaPeriods[note
							+ uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].fineTune];

			if (effectNumber != MOD32_TONEPORTAMENTO
					&& effectNumber != MOD32_PORTAMENTOVOLUMESLIDE)
				uPlayer.Mod_player.lastAmigaPeriod[channel] =
						uPlayer.Mod_player.amigaPeriod[channel];

			if (!(uPlayer.Mod_player.waveControl[channel] & 0x80))
				uPlayer.Mod_player.vibratoPos[channel] = 0;
			if (!(uPlayer.Mod_player.waveControl[channel] & 0x08))
		...
		case MOD32_TONEPORTAMENTO:
			if (effectParameter)
				uPlayer.Mod_player.portamentoSpeed[channel] = effectParameter;
			uPlayer.Mod_player.portamentoNote[channel] =
					uPlayer.Mod_player.amigaPeriod[channel];
			note = NONOTE_MOD;
			break;
		...
		case MOD32_VIBRATO:
			if (effectParameterX)
				uPlayer.Mod_player.vibratoSpeed[channel] = effectParameterX;
			if (effectParameterY)
				uPlayer.Mod_player.vibratoDepth[channel] = effectParameterY;
			break;
		...
		case MOD32_TREMOLO:
			if (effectParameterX)
				uPlayer.Mod_player.tremoloSpeed[channel] = effectParameterX;
			if (effectParameterY)
				uPlayer.Mod_player.tremoloDepth[channel] = effectParameterY;
			break;
		...
		case MOD32_SETSAMPLEOFFSET:
			...
			if (sampleOffset
					> uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].length)
				sampleOffset =
						uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].length;
			break;
		...
		case MOD32_SETVOLUME:
			if (effectParameter > 64)
				uPlayer.Mod_player.volume[channel] = 64;
			else
				uPlayer.Mod_player.volume[channel] = effectParameter;
			break;
		...
		case 0xE:
			...
			case MOD32_SETFINETUNE:
				uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].fineTune =
						effectParameterY;
				if (uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].fineTune
						> 7)
					uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].fineTune -=
							16;
				break;
				...
				if (effectParameterY) {
					if (uPlayer.Mod_player.patternLoopCount[channel])
						...
						uPlayer.Mod_player.patternLoopCount[channel] =
								effectParameterY;
					if (uPlayer.Mod_player.patternLoopCount[channel])
						uPlayer.Mod_player.row =
								uPlayer.Mod_player.patternLoopRow[channel] - 1;
				} else
					uPlayer.Mod_player.patternLoopRow[channel] =
							uPlayer.Mod_player.row;
				break;
			...
			case MOD32_FINEVOLUMESLIDEUP:
				...
				if (uPlayer.Mod_player.volume[channel] > 64)
					uPlayer.Mod_player.volume[channel] = 64;
				break;

			case MOD32_FINEVOLUMESLIDEDOWN:
				uPlayer.Mod_player.volume[channel] -= effectParameterY;
				if (uPlayer.Mod_player.volume[channel] < 0)
					uPlayer.Mod_player.volume[channel] = 0;
				break;
		...
		case MOD32_SETSPEED:
			if (effectParameter < 0x20)
				...
				uPlayer.Mod_player.samplesPerTick = SAMPLERATE
						/ (2 * effectParameter / 5);
			break;
		}
		...
		if (note != NONOTE_MOD
				|| uPlayer.Mod_player.lastAmigaPeriod[channel]
						&& effectNumber != MOD32_VIBRATO
						&& effectNumber != MOD32_VIBRATOVOLUMESLIDE
						&& !(effectNumber == 0xE
								&& effectParameterX == MOD32_NOTEDELAY))
			Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
					/ uPlayer.Mod_player.lastAmigaPeriod[channel];
		...
		if (sampleNumber)
			Mixer.channelSampleNumber[channel] =
					uPlayer.Mod_player.lastSampleNumber[channel];
	...
	for (channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
		...
		if (uPlayer.Mod_player.lastAmigaPeriod[channel]) {
				...
				if (effectParameter)
					...
					case 0:
						Mixer.channelFrequency[channel] =
								uPlayer.Mod_player.amiga
										/ uPlayer.Mod_player.lastAmigaPeriod[channel];
						break;
					case 1:
						tempNote =
								uPlayer.Mod_player.lastNote[channel]
										+ effectParameterX * 8
										+ uMod.Mod.samples[uPlayer.Mod_player.lastSampleNumber[channel]].fineTune;
						if (tempNote < 296)
							Mixer.channelFrequency[channel] =
									uPlayer.Mod_player.amiga
											/ amigaPeriods[tempNote];
						break;
					case 2:
						tempNote =
								uPlayer.Mod_player.lastNote[channel]
			...
			case MOD32_PORTAMENTOUP:
				...
				if (uPlayer.Mod_player.lastAmigaPeriod[channel] < 113)
					uPlayer.Mod_player.lastAmigaPeriod[channel] = 113;
				Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
						/ uPlayer.Mod_player.lastAmigaPeriod[channel];
				break;
			...
			case MOD32_PORTAMENTODOWN:
				uPlayer.Mod_player.lastAmigaPeriod[channel] += effectParameter;
				if (uPlayer.Mod_player.lastAmigaPeriod[channel] > 856)
					uPlayer.Mod_player.lastAmigaPeriod[channel] = 856;
				Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga
						/ uPlayer.Mod_player.lastAmigaPeriod[channel];
				break;
			...
			case MOD32_PORTAMENTOVOLUMESLIDE:
				portamento(channel);
				uPlayer.Mod_player.volume[channel] += effectParameterX
						- effectParameterY;
				if (uPlayer.Mod_player.volume[channel] < 0)
					uPlayer.Mod_player.volume[channel] = 0;
				else if (uPlayer.Mod_player.volume[channel] > 64)
					uPlayer.Mod_player.volume[channel] = 64;
...
void mod_player() {
	...
	if (uPlayer.Mod_player.tick == uPlayer.Mod_player.speed) {
		...
		if (uPlayer.Mod_player.patternDelay) {
			...
			if (uPlayer.Mod_player.orderIndex
					!= uPlayer.Mod_player.oldOrderIndex)
...
void mod_mixer() {
	...
	for (channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
		...
		if (!Mixer.channelFrequency[channel]
				|| !uMod.Mod.samples[Mixer.channelSampleNumber[channel]].length)
			continue;
		...
		next = FatBuffer.channels[channel][(samplePointer + 1
				- FatBuffer.samplePointer[channel]) & FATBUFFERSIZE - 1]; //instead of FatBufferSIZE I've put 0x400
