...
// Amiga periods
static const uint16_t amigaPeriods[110] = { 27392, 25856, 24384, 23040, 21696,
		20480, 19328, 18240, 17216, 16256, 15360,
		14496, // 0
		13696, 12928, 12192, 11520, 10848, 10240, 9664, 9120, 8608, 8128, 7680,
		7248, // 1
...
static const uint16_t fineTuneToHz[16] = { 8363, 8413, 8463, 8529, 8581, 8651,
		8723, 8757, 7895, 7941, 7985, 8046, 8107, 8169, 8232, 8280 };

static const uint8_t sine[64] = { 0, 24, 49, 74, 97, 120, 141, 161, 180, 197,
		212, 224, 235, 244, 250, 253, 255, 253, 250, 244, 235, 224, 212, 197,
...
static void loadHeader() {
	...
	if (temp16 & 64)
		uMod.S3m.fastVolumeSlides = true;             // Fast volume slides flag
	f_read(&file, &temp16, 2, &count);         // Created with tracker / version
	if ((temp16 & 0xFFF) == 300)
		uMod.S3m.fastVolumeSlides = true; // Fast volume slides for Scream Tracker 3.00
	...
	// Load parapointers
	f_read(&file, uMod.S3m.instrumentParapointers,
			uMod.S3m.numberOfInstruments * 2, &count);
	...
	if (defaultPanning == 0xFC) {
		for (i = 0; i < CHANNELS; i++) {
			...
			if ((temp8 & 0x20) && uMod.S3m.channelRemapping[i] < CHANNELS)
				Mixer.channelPanning[uMod.S3m.channelRemapping[i]] = temp8
						& 0xF;
		}
	}
	...
	for (i = 0; i < uMod.S3m.numberOfInstruments; i++) {
		...
		if (!uMod.S3m.instruments[i].middleC)
			uMod.S3m.instruments[i].middleC = 8363; // Avoid division by zero
...
static void loadSamples() {
	...
	for (i = 0; i < uMod.S3m.numberOfInstruments; i++) {
		...
		if (uMod.S3m.instruments[i].length) {
			...
			if (uMod.S3m.instruments[i].loop
					&& uMod.S3m.instruments[i].loopEnd
							- uMod.S3m.instruments[i].loopBegin > 2) {
				Mixer.sampleLoopBegin[i] = fileOffset
						+ uMod.S3m.instruments[i].loopBegin;
				Mixer.sampleLoopLength[i] = uMod.S3m.instruments[i].loopEnd
						- uMod.S3m.instruments[i].loopBegin;
				Mixer.sampleLoopEnd[i] = fileOffset
						+ uMod.S3m.instruments[i].loopEnd;
			} else {
...
static void loadPattern(uint8_t pattern) {
	...
	memset(uPlayer.S3m_player.currentPattern.instrumentNumber, 0,
			ROWS * CHANNELS);
	...
	memset(uPlayer.S3m_player.currentPattern.effectParameter, 0,
			ROWS * CHANNELS);
		...
		if (temp8) {
			...
			if (channel == 0XFF) //no channel
				...
				if (temp8 & 32) {
					...
					switch (temp16) {
					case 255:
						uPlayer.S3m_player.currentPattern.note[row][channel] =
								NONOTE;
						break;
					case 254:
						uPlayer.S3m_player.currentPattern.note[row][channel] =
					...
					default:
						uPlayer.S3m_player.currentPattern.note[row][channel] =
								(temp16 >> 4) * 12 + (temp16 & 0xF);
					}
					f_read(&file,
							&uPlayer.S3m_player.currentPattern.instrumentNumber[row][channel],
							1, &count);
				}

				if (temp8 & 64)
					f_read(&file,
							&uPlayer.S3m_player.currentPattern.volume[row][channel],
							1, &count);

				if (temp8 & 128) {
					f_read(&file,
							&uPlayer.S3m_player.currentPattern.effectNumber[row][channel],
							1, &count);
					f_read(&file,
							&uPlayer.S3m_player.currentPattern.effectParameter[row][channel],
							1, &count);
				}
			}
		} else
...
static void portamento(uint8_t channel) {
	if (uPlayer.S3m_player.lastAmigaPeriod[channel]
			< uPlayer.S3m_player.portamentoNote[channel]) {
		uPlayer.S3m_player.lastAmigaPeriod[channel] +=
				uPlayer.S3m_player.portamentoSpeed[channel] << 2;
		if (uPlayer.S3m_player.lastAmigaPeriod[channel]
				> uPlayer.S3m_player.portamentoNote[channel])
			uPlayer.S3m_player.lastAmigaPeriod[channel] =
					uPlayer.S3m_player.portamentoNote[channel];
	...
	Mixer.channelFrequency[channel] = uPlayer.S3m_player.amiga
			/ uPlayer.S3m_player.lastAmigaPeriod[channel];
}
...
static void vibrato(uint8_t channel, bool fine) {
	...
	if (uPlayer.S3m_player.vibratoPos[channel] >= 0)
		Mixer.channelFrequency[channel] = uPlayer.S3m_player.amiga
				/ (uPlayer.S3m_player.lastAmigaPeriod[channel] + delta);
	else
		Mixer.channelFrequency[channel] = uPlayer.S3m_player.amiga
				/ (uPlayer.S3m_player.lastAmigaPeriod[channel] - delta);

	uPlayer.S3m_player.vibratoPos[channel] +=
			uPlayer.S3m_player.vibratoSpeed[channel];
	if (uPlayer.S3m_player.vibratoPos[channel] > 31)
		uPlayer.S3m_player.vibratoPos[channel] -= 64;
}
...
static void tremolo(uint8_t channel) {
	...
	if (uPlayer.S3m_player.tremoloPos[channel] >= 0) {
		if (uPlayer.S3m_player.volume[channel] + delta > 64)
			delta = 64 - uPlayer.S3m_player.volume[channel];
		Mixer.channelVolume[channel] = uPlayer.S3m_player.volume[channel]
				+ delta;
	} else {
		if (uPlayer.S3m_player.volume[channel] - delta < 0)
			delta = uPlayer.S3m_player.volume[channel];
		Mixer.channelVolume[channel] = uPlayer.S3m_player.volume[channel]
				- delta;
	}

	uPlayer.S3m_player.tremoloPos[channel] +=
			uPlayer.S3m_player.tremoloSpeed[channel];
	if (uPlayer.S3m_player.tremoloPos[channel] > 31)
		uPlayer.S3m_player.tremoloPos[channel] -= 64;
}
...
static void volumeSlide(uint8_t channel) {
	if (!(uPlayer.S3m_player.lastVolumeSlide[channel] & 0xF))
		uPlayer.S3m_player.volume[channel] +=
				uPlayer.S3m_player.lastVolumeSlide[channel] >> 4;
	if (!(uPlayer.S3m_player.lastVolumeSlide[channel] >> 4))
		uPlayer.S3m_player.volume[channel] -=
				uPlayer.S3m_player.lastVolumeSlide[channel] & 0xF;

	if (uPlayer.S3m_player.volume[channel] > 64)
		uPlayer.S3m_player.volume[channel] = 64;
	else if (uPlayer.S3m_player.volume[channel] < 0)
		uPlayer.S3m_player.volume[channel] = 0;
	Mixer.channelVolume[channel] = uPlayer.S3m_player.volume[channel];
}
...
static void processRow() {
	...
	for (channel = 0; channel < uMod.S3m.numberOfChannels; channel++) {
		...
		note =
				uPlayer.S3m_player.currentPattern.note[uPlayer.S3m_player.lastRow][channel];
		instrumentNumber =
				uPlayer.S3m_player.currentPattern.instrumentNumber[uPlayer.S3m_player.lastRow][channel];
		volume =
				uPlayer.S3m_player.currentPattern.volume[uPlayer.S3m_player.lastRow][channel];
		effectNumber =
				uPlayer.S3m_player.currentPattern.effectNumber[uPlayer.S3m_player.lastRow][channel];
		effectParameter =
				uPlayer.S3m_player.currentPattern.effectParameter[uPlayer.S3m_player.lastRow][channel];
		effectParameterX = effectParameter >> 4;
		effectParameterY = effectParameter & 0xF;
		sampleOffset = 0;

		if (instrumentNumber) {
			uPlayer.S3m_player.lastInstrumentNumber[channel] = instrumentNumber
					- 1;
			if (!(effectParameter == 0x13 && effectParameterX == S3M32_NOTEDELAY)) {
				uPlayer.S3m_player.volume[channel] =
						uMod.S3m.instruments[uPlayer.S3m_player.lastInstrumentNumber[channel]].volume;
			}
		}

		if (note < NONOTE) {
			uPlayer.S3m_player.lastNote[channel] = note;
			uPlayer.S3m_player.amigaPeriod[channel] =
					amigaPeriods[note] * 8363
							/ uMod.S3m.instruments[uPlayer.S3m_player.lastInstrumentNumber[channel]].middleC;

			if (effectNumber != S3M32_TONEPORTAMENTO
					&& effectNumber != S3M32_PORTAMENTOVOLUMESLIDE)
				uPlayer.S3m_player.lastAmigaPeriod[channel] =
						uPlayer.S3m_player.amigaPeriod[channel];

			if (!(uPlayer.S3m_player.waveControl[channel] & 0x80))
				uPlayer.S3m_player.vibratoPos[channel] = 0;
			if (!(uPlayer.S3m_player.waveControl[channel] & 0x08))
		...
		if (volume <= 0x40)
			uPlayer.S3m_player.volume[channel] = volume;
		if (note == KEYOFF)
			uPlayer.S3m_player.volume[channel] = 0;
		...
		case S3M32_VOLUMESLIDE:
			if (effectParameter)
				uPlayer.S3m_player.lastVolumeSlide[channel] = effectParameter;
		...
		case S3M32_PORTAMENTODOWN:
			if (effectParameter)
				uPlayer.S3m_player.lastPortamento[channel] = effectParameter;
			if (uPlayer.S3m_player.lastPortamento[channel] >> 4 == 0xF)
				uPlayer.S3m_player.lastAmigaPeriod[channel] +=
						(uPlayer.S3m_player.lastPortamento[channel] & 0xF) << 2;
			if (uPlayer.S3m_player.lastPortamento[channel] >> 4 == 0xE)
				uPlayer.S3m_player.lastAmigaPeriod[channel] +=
						uPlayer.S3m_player.lastPortamento[channel] & 0xF;
			break;
		...
		case S3M32_TONEPORTAMENTO:
			if (effectParameter)
				uPlayer.S3m_player.portamentoSpeed[channel] = effectParameter;
			uPlayer.S3m_player.portamentoNote[channel] =
					uPlayer.S3m_player.amigaPeriod[channel];
			retriggerSample = false;
			break;
		...
		case S3M32_VIBRATO:
			if (effectParameterX)
				uPlayer.S3m_player.vibratoSpeed[channel] = effectParameterX;
			if (effectParameterY)
				uPlayer.S3m_player.vibratoDepth[channel] = effectParameterY;
			break;

		case S3M32_TREMOR:
			if (effectParameter)
				uPlayer.S3m_player.tremorOnOff[channel] = effectParameter;
			tremor(channel);
			break;
		...
		case S3M32_SETSAMPLEOFFSET:
			...
			if (sampleOffset
					> uMod.S3m.instruments[uPlayer.S3m_player.lastInstrumentNumber[channel]].length)
				sampleOffset =
						uMod.S3m.instruments[uPlayer.S3m_player.lastInstrumentNumber[channel]].length;
			break;
		...
		case S3M32_RETRIGGERNOTEVOLUMESLIDE:
			if (effectParameter) {
				uPlayer.S3m_player.retriggerVolumeSlide[channel] =
						effectParameterX;
		...
		case S3M32_TREMOLO:
			if (effectParameterX)
				uPlayer.S3m_player.tremoloSpeed[channel] = effectParameterX;
			if (effectParameterY)
				uPlayer.S3m_player.tremoloDepth[channel] = effectParameterY;
			break;
				...
				if (effectParameterY) {
					if (uPlayer.S3m_player.patternLoopCount[channel])
						...
						uPlayer.S3m_player.patternLoopCount[channel] =
								effectParameterY;
					if (uPlayer.S3m_player.patternLoopCount[channel])
						uPlayer.S3m_player.row =
								uPlayer.S3m_player.patternLoopRow[channel] - 1;
				} else
					uPlayer.S3m_player.patternLoopRow[channel] =
							uPlayer.S3m_player.row;
				break;
		...
		case S3M32_SETTEMPO:
			uPlayer.S3m_player.samplesPerTick = SAMPLERATE
					/ ((effectParameter << 1) / 5);
			break;
		...
		if (retriggerSample
				|| uPlayer.S3m_player.lastAmigaPeriod[channel]
						&& effectNumber != S3M32_VIBRATO
						&& effectNumber != S3M32_VIBRATOVOLUMESLIDE
						&& !(effectNumber == 0x13
								&& effectParameterX == S3M32_NOTEDELAY)
						&& effectNumber != S3M32_FINEVIBRATO)
			Mixer.channelFrequency[channel] = uPlayer.S3m_player.amiga
					/ uPlayer.S3m_player.lastAmigaPeriod[channel];

		if (instrumentNumber)
			Mixer.channelSampleNumber[channel] =
					uPlayer.S3m_player.lastInstrumentNumber[channel];
		...
		if (uPlayer.S3m_player.lastAmigaPeriod[channel]) {
			...
			case S3M32_PORTAMENTODOWN:
				if (uPlayer.S3m_player.lastPortamento[channel] < 0xE0)
					uPlayer.S3m_player.lastAmigaPeriod[channel] +=
							uPlayer.S3m_player.lastPortamento[channel] << 2;
				Mixer.channelFrequency[channel] = uPlayer.S3m_player.amiga
						/ uPlayer.S3m_player.lastAmigaPeriod[channel];
				break;
			...
			case S3M32_ARPEGGIO:
				if (effectParameter)
					...
					case 0:
						Mixer.channelFrequency[channel] =
								uPlayer.S3m_player.amiga
										/ uPlayer.S3m_player.lastAmigaPeriod[channel];
						break;
					case 1:
						Mixer.channelFrequency[channel] =
								uPlayer.S3m_player.amiga
										/ amigaPeriods[uPlayer.S3m_player.lastNote[channel]
												+ effectParameterX];
						break;
					case 2:
						Mixer.channelFrequency[channel] =
			...
			case S3M32_RETRIGGERNOTEVOLUMESLIDE:
				if (!uPlayer.S3m_player.retriggerSpeed[channel])
					break;
				if (!(uPlayer.S3m_player.tick
						% uPlayer.S3m_player.retriggerSpeed[channel])) {
...
void s3m_player() {
	...
	if (uPlayer.S3m_player.tick == uPlayer.S3m_player.speed) {
		...
		if (uPlayer.S3m_player.row == ROWS) {
			...
			if (uPlayer.S3m_player.orderIndex == uMod.S3m.songLength)
			{
				...
				/* Set flag to start next module is enabled */
				playNextMod = true;
			}
			uPlayer.S3m_player.row = 0;
		}
		...
		if (uPlayer.S3m_player.patternDelay) {
			...
			if (uPlayer.S3m_player.orderIndex
					!= uPlayer.S3m_player.oldOrderIndex) {
	...
	sprintf((char*) buffer, "%02u/%02u/%02u", uPlayer.Mod_player.row,
...
void s3m_mixer() {
	...
	for (channel = 0; channel < uMod.S3m.numberOfChannels; channel++) {
		...
		if (!Mixer.channelFrequency[channel]
				|| !uMod.S3m.instruments[Mixer.channelSampleNumber[channel]].length)
			continue;
		...
		if (Mixer.sampleLoopLength[Mixer.channelSampleNumber[channel]]) {
			...
			if (samplePointer
					>= Mixer.sampleLoopEnd[Mixer.channelSampleNumber[channel]]) {
				Mixer.channelSampleOffset[channel] -=
						Mixer.sampleLoopLength[Mixer.channelSampleNumber[channel]]
								<< DIVIDER;
				samplePointer -=
						Mixer.sampleLoopLength[Mixer.channelSampleNumber[channel]];
			}
			...
			if (samplePointer
					>= Mixer.sampleEnd[Mixer.channelSampleNumber[channel]]) {
		...
		if (samplePointer < FatBuffer.samplePointer[channel]
				|| samplePointer
						>= FatBuffer.samplePointer[channel] + FATBUFFERSIZE - 1
				|| Mixer.channelSampleNumber[channel]
						!= FatBuffer.channelSampleNumber[channel]) {

			fatBufferSize = Mixer.sampleEnd[Mixer.channelSampleNumber[channel]]
					- samplePointer + 1;
			if (fatBufferSize > FATBUFFERSIZE)
		...
		current = FatBuffer.channels[channel][(samplePointer
				- FatBuffer.samplePointer[channel]) & FATBUFFERSIZE - 1]; //instead of FatBufferSIZE I've put 0x400 so that I can use non 2 power sizes
		next = FatBuffer.channels[channel][(samplePointer + 1
				- FatBuffer.samplePointer[channel]) & FATBUFFERSIZE - 1];
